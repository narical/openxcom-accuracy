...
git config alias.$1-version-bump '!'"f() { vim -T dumb --noplugin -n -c '%s/\\(\\d\\+[.,]\\d\\+[.,]\\)\\(\\d\\+\\)/\\=submatch(1).eval(submatch(2)+1)/g' -c '%s/(v\\d\\d\\d\\d-\\d\\d-\\d\\d)/\\=\"(v\".strftime(\"%Y-%m-%d\").\")\"/g' -c ':wq' src/version.h && git add src/version.h; }; f"
